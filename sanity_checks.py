    ...
    for i, employee in enumerate(employee_info):
        ...
        if type(employee["name"]) is not str:
            raise EmployeeDictError("Employee: {}'s 'name' is not string. "
                                    ...
                                    "Please contact a representative of "
                                    "SlobodinScheduling for assistance.".format(i, i, type(employee["name"])))
        ...
        for sp in employee["shift_pref"]:
            ...
            if len(sp) != num_shifts:
                raise EmployeeDictError("Employee: {}'s 'shift_pref' contains a list of length ({})"
                                        ...
                                        "SlobodinScheduling for assistance."
                                        .format(employee["name"], len(sp), num_shifts))
            index = -1
            for s in sp:
                index += 1
                ...
                if s["pref"] not in (0, 1, 2):
                    raise EmployeeDictError("Employee: {}'s 'shift_pref'[{}]['pref'] is invalid. "
                                            ...
                                            "Please contact a representative of SlobodinScheduling for assistance."
                                            .format(employee["name"], index, employee["name"], index, s["pref"]))
...
#Check that 'management_data' is correct format
    if type(management_data) is not list:
        raise ManagementDictError("'Management_data' is not a list. "
                                  "Recieved 'Management_data' as {}. "
                                  "Please contact a representative of SlobodinScheduling for assistance."
                                  .format(type(management_data)))
#Check if 'management_data' contains information on the correct number of roles
    if len(management_data) != config_data['num_roles']:
        raise ManagementDictError("'Management_data' is of length ({}) when expected is length ({}). "
                                  "Please contact a representative of SlobodinScheduling for assistance."
                                  .format(len(management_data), config_data['num_roles']))
#Check that 'management_data'[role] is correct format
    for role in range(config_data['num_roles']):
        if type(management_data[role]) is not dict:
            raise ManagementDictError("'Management_data'[{}] is not a dict. "
                                      "Recieved 'Management_data'[{}] as {}. "
                                      "Please contact a representative of SlobodinScheduling for assistance."
                                      .format(str(role), str(role), type(management_data[role])))
        ...
        for day in range(config_data['num_days']):
            ...
            if day not in management_data[role]:
                ...
                raise ManagementDictError( "'Management_data'[{}] does not contain information on day ({}). "
                                           "Please contact a representative of SlobodinScheduling for assistance."
                                           .format(str(role), str(day)))
            #Check if management_data[role][day] is a dict
            if type(management_data[role][day]) is not dict:
                raise ManagementDictError("'Managment_data'[{}][{}] is not a dict. "
                                          "Recieved 'Managment_data'[{}][{}] as {}. "
                                          "Please contact a representative of SlobodinScheduling for assistance."
                                          .format(str(role), str(day), str(role), str(day), type(management_data[role][day])))
            #Check if 'management_data'[role][day]['num_employees'] is a list
            if type(management_data[role][day]['num_employees']) is not list:
                raise ManagementDictError("'Manangemet_data'[{}][{}]['num_employees'] is not a list. "
                                          "Recieved 'Manangemet_data'[{}][{}]['num_employees'] as {}. "
                                          "Please contact a representative of SlobodinScheduling for assistance."
                                          .format(str(role), str(day), str(role), str(day), type(management_data[role][day]['num_employees'])))
            ...
            if len(management_data[role][day]['num_employees']) != config_data['num_shifts']:
                ...
                raise ManagementDictError("'Manangemet_data'[{}][{}]['num_employees'] is length ({}) "
                                          "when expected is length ({}). "
                                          "Please contact a representative of SlobodinScheduling for assistance."
                                          .format(str(role), str(day), len(management_data[role][day]['num_employees']), str(config_data['num_shifts'])))
            ...
            if type(management_data[role][day]['shift_times']) is not list:
                raise ManagementDictError("'Management_data[{}][{}]['shift_times] is not a list. "
                                          "Recieved 'Management_data[{}][{}]['shift_times] as {}. "
                                          "Please contact a representative of SlobodinScheduling for assistance."
                                          .format(str(role), str(day), str(role), str(day), type(management_data[role][day]['shift_times'])))
            #Check if 'management_data'[role][day]['shift_times'] is the correct length (number of shifts)
            if len(management_data[role][day]['shift_times']) != config_data['num_shifts']:
                raise ManagementDictError("'Manangemet_data[{}][{}]['shift_times'] is length ({}) "
                                          "when expected is length ({}). "
                                          "Please contact a representative of SlobodinScheduling for assistance."
                                          .format(str(role), str(day), len(management_data[role][day]['num_employees']), str(config_data['num_shifts'])))
            ...
            for shift in range(config_data['num_shifts']):
                ...
                if type(management_data[role][day]['num_employees'][shift]) is not int:
                    raise ManagementDictError("'Management_data'[{}][{}]['num_employees'][{}] is not an integer. "
                                              "Recieved 'Management_data'[{}][{}]['num_employees'][{}] as {}. "
                                              "Please contact a representative of SlobodinScheduling for assistance."
                                              .format(str(role), str(day), str(shift), str(role), str(day), str(shift), type(management_data[role][day]['num_employees'][shift])))
                if management_data[role][day]['num_employees'][shift] < 0:
                    raise ManagementDictError("'Management_data'[{}][{}]['num_employees'][{}] is an negative integer. "
                                              "Please contact a representative of SlobodinScheduling for assistance."
                                              .format(str(role), str(day), str(shift)))
                ...
                if type(management_data[role][day]['shift_times'][shift]) is not str:
                    raise ManagementDictError("'Management_data'[{}][{}]['shift_times'][{}] is not an string. "
                                              "Recieved 'Management_data'[{}][{}]['shift_times'][{}] as {}. "
                                              "Please contact a representative of SlobodinScheduling for assistance."
                                              .format(str(role), str(day), str(shift), str(role), str(day), str(shift), type(management_data[role][day]['shift_times'][shift]) ))
...
def check_management_employee_constraints(config_data, management_data, employee_data):
    ...
    if required_num_employees > employees_maximum_shift_availability:
        raise ConstraintError("In Management Tab, required number of employees to work is ({}). "
                              "This value exceeds the maximum availability of employees in Employee Information Tab: ({})."
    ...
    if required_num_employees < employees_minimum_shift_availability: 
        raise ConstraintError("In Employee Information Tab, the minimum availability of your employees is ({}). "
                              "This value exceeds the required number of employees to work in Management Tab: ({})."
                              "\n\nPlease reduce your employees' minimum availability in Employee Information Tab "
    ...
    for role in range(config_data['num_roles']):
        ...
        if employees_maximum_shift_availability_per_role < required_num_employees_per_role:
            raise ConstraintError("In Management Tab, required number of employees to work as {} role is ({}). "
                                  "This value exceeds the maximum availability of employees who can work as {} role "
                                  "in Employee Information Tab: ({}).\n\n"
                                  "Please reduce the required number of employees to work as {} role in Management Tab "
                                  "or increase the number of employees who can work as {} role in Employee Tab."
        ...
        for day in range(config_data['num_days']):
            for shift in range(config_data['num_shifts']):
                if num_employees_able_to_work_role < management_data[role][day]['num_employees'][shift]:
                    raise ConstraintError("In Management Tab, required number of employees to work as {} role on {} "
                                          "during {} shift is ({}). This value exceeds the number of employees "
                                          "who can work as {} role in Employee Tab: ({})."
                                          "\n\nPlease reduce the required number of employees to work as {} role on {} "
    ...
    for role in range(config_data['num_roles']):
        ...
        for day in range(config_data['num_days']):
            ...
            if num_employees_able_to_work_role < num_employees_required_to_work_role:
                raise ConstraintError("In Management Tab, required number of employees to work as {} role on {} is ({}). "
                                      "This value exceeds the number of employees who can work as {} role in "
                                      "Employee Tab: ({}).\n\nPlease reduce the number of employees required to work "
    ...
    for day in range(config_data['num_days']):
        ...
        if total_employees_required > config_data['num_employees']:
            raise ConstraintError("In Management Tab, required number of employees to work on {} is ({}). "
                                  "This value exceeds the total number of employees: ({})."
                                  "\n\nPlease reduce the required number of employees to work on {} in Management Tab "
                                  "or increase total number of employees in Employee Tab."
                                  .format(str(config_data['day_names'][day]), str(total_employees_required), str(config_data['num_employees']), str(config_data['day_names'][day])))
